...
export default function LandingHeroSection() {
  return (
    <section className="overflow-clip bg-gradient-to-b from-brand-50 to-white">
      <Container className="py-10 sm:py-14 lg:py-20">
        <div className="grid items-center gap-9 lg:grid-cols-[1.05fr_.95fr] lg:gap-12">
          <div className="min-w-0">
            <p className="inline-flex rounded-full border border-brand-100 bg-white px-3 py-1.5 text-xs font-bold text-brand-700 sm:text-sm">
            ...
            <ul className="mt-7 grid gap-2.5 text-sm text-slate-600 sm:grid-cols-3">
              {benefits.map((benefit) => (
                <li key={benefit} className="flex items-start gap-2">
                  <CircleCheck className="mt-0.5 size-4 shrink-0 text-brand-600" />
                  <span>{benefit}</span>
                </li>
              ))}
            </ul>
          ...
          <div className="relative mx-auto w-full max-w-xl lg:max-w-none">
            <div className="relative aspect-[4/3] overflow-hidden rounded-[2rem] bg-slate-100 shadow-2xl shadow-brand-500/10">
              <img
            ...
            <div className="absolute -bottom-4 left-3 right-3 rounded-2xl border bg-white/95 p-4 shadow-lg backdrop-blur sm:left-auto sm:right-5 sm:w-64">
              <p className="text-xs font-semibold text-slate-500">Tiến độ tuần này</p>
              <p className="mt-1 text-xl font-bold">+18% hoàn thành</p>
